...
interface AnyTypeDefintionProps {
    ruleData:any
}
 
const AnyTypeDefinition: FunctionComponent<AnyTypeDefintionProps> = ({ruleData}) => {
	...
	useEffect(() => {
		if(method === "update")
		{
			...
			let structuredRuleData = {
				...ruleData,
				likeExpression : parameters?.likeExpression?.value,
			}
    ...
    return ( 
        <>
            <ModalContent className="customModal">
				...
				/>
				<LabeledInput
					placeholder="Like Expression"
					label="Like Expression"
					name="likeExpression"
					onChange={handleInput}
					value={ruleDetails.likeExpression || ""}
					style={{ margin: "10px 0px" }}
...
export default AnyTypeDefinition;