...
export default class ClashReviewApi extends HelperMethods {
	...
	private static _RMS_BASE_URL: string = `https://${process.env.REACT_APP_IMJS_URL_PREFIX}connect-designvalidationrulemanagement.bentley.com/v3`; // QA - RMS Endpoint
	private static _RAS_BASE_URL: string = `https://${process.env.REACT_APP_IMJS_URL_PREFIX}connect-resultsanalysisservice.bentley.com/v2`; // QA - RAS Endpoint
	...
	private static async getLatestChangeSetIdForIModel(iModelId: string) {
		const response = await fetch(`https://${process.env.REACT_APP_IMJS_URL_PREFIX}api.bentley.com/imodels/${iModelId}/changesets?$top=1&$orderBy=index desc`, {
			headers: {
	...
	public static async getClashTestDetailById(projectId: string, testId: string) {
		const response = await fetch(
			`https://${process.env.REACT_APP_IMJS_URL_PREFIX}connect-designvalidationrulemanagement.bentley.com/v3/contexts/${projectId}/tests/${testId}`,
			{
				headers: {
					Authorization: ClashReviewApi._accessToken,
					"Include-User-Metadata": "false",
	...
	public static async createClashDetectionTest(projectId: string, data: any) {
		const response = await fetch(`https://${process.env.REACT_APP_IMJS_URL_PREFIX}connect-designvalidationrulemanagement.bentley.com/v3/contexts/${projectId}/tests`, {
			method: "POST",
			headers: {
				Authorization: ClashReviewApi._accessToken,
				"Content-Type": "application/json",
	...
	public static async getModelsAndCategories(iModelId: string, projectId: string) {
		if (ClashReviewApi._modelsAndCategories[iModelId] === undefined) {
			const response = await fetch(
				`https://${process.env.REACT_APP_IMJS_URL_PREFIX}api.bentley.com/clashdetection/modelsAndCategories/imodels/${iModelId}?projectId=${projectId}`,
				{
					headers: {
						Authorization: ClashReviewApi._accessToken,
					},
	...
	public static async getMappingAndGrouping(iModelId: string) {
		if (ClashReviewApi._mappings[iModelId] === undefined) {
			const response = await fetch(`https://${process.env.REACT_APP_IMJS_URL_PREFIX}api.bentley.com/insights/reporting/datasources/imodels/${iModelId}/mappings`, {
				headers: {
					Authorization: ClashReviewApi._accessToken,
				},
			});
	...
	public static async getGroupsForMappingId(iModelId: string, mappingId: string) {
		if (ClashReviewApi._groupings[mappingId] === undefined) {
			const response = await fetch(
				`https://${process.env.REACT_APP_IMJS_URL_PREFIX}api.bentley.com/insights/reporting/datasources/imodels/${iModelId}/mappings/${mappingId}/groups`,
				{
					headers: {
						Authorization: ClashReviewApi._accessToken,
					},
